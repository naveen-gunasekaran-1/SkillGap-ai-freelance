...
import { FlatList, Pressable, Text, TextInput, View, RefreshControl } from 'react-native';
...
export default function JobsScreen(): React.JSX.Element {
  ...
  const load = useCallback(async () => {
    ...
    const rows: JobRow[] = res.data.jobs.map((j) => {
      ...
      return {
        ...
        ...(typeof j.applicantCount === 'number' ? { applicantCount: j.applicantCount } : {}),
      };
    });
    setJobs(rows);
  }, [isCompany, search]);
  ...
  const scoreColor = (v: number) =>
    v < 40 ? t.colors.error : v <= 70 ? t.colors.warning : t.colors.success;
      ...
      >
        ...
        >
          <Text style={{ ...t.typography.h2, color: t.colors.textPrimary }}>
            {isCompany ? 'My Jobs' : 'Jobs'}
          </Text>
          {isCompany ? (
            <Pressable
              onPress={() => router.push('/company/job-form')}
              style={({ pressed }) => ({
                width: 42,
                height: 42,
                borderRadius: 21,
        ...
        ListEmptyComponent={
          !loading ? (
            ...
            >
              ...
              >
                {isCompany ? 'No posted jobs' : 'No jobs found'}
              </Text>
              <Text style={{ ...t.typography.body, color: t.colors.textSecondary, marginTop: 8 }}>
                Pull to refresh or adjust search
              </Text>
            </View>
          ) : null
        }
          ...
          >
            ...
            >
              <View style={{ flex: 1, paddingRight: t.spacing.md }}>
                ...
                >
                  <Text
                    style={{ ...t.typography.body, fontWeight: '700', color: t.colors.primary }}
                  >
                    {isCompany && item.applicantCount != null
              ...
              <View
                style={{
                  ...
                  borderRadius: 28,
                  borderWidth: 4,
                  borderColor: isCompany ? t.colors.primary : scoreColor(item.score),
                  alignItems: 'center',
                  justifyContent: 'center',
                  backgroundColor: t.colors.surface,
                }}
              >
                ...
                >
                  {isCompany ? (item.applicantCount ?? 0) : `${item.score}%`}
                </Text>
              </View>
            </View>
          </Pressable>
