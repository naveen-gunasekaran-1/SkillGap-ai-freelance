...
export function RegisterPage(): React.JSX.Element {
  ...
  const setProfileField = (key: keyof typeof profile, val: string) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (role === 'CANDIDATE') {
      ...
      const hasEducation = education.some(
        (e) => e.school.trim() && e.degree.trim() && e.startYear.trim(),
      );
      const hasInternship = internships.some(
        (i) => i.company.trim() && i.role.trim() && i.startDate.trim(),
      );
          ...
          <div className="mb-8 flex items-center justify-center gap-2">
            {[1, 2, 3, 4].map((s) => (
              <div key={s} className="flex items-center gap-2">
                <div
                  className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-semibold transition-all duration-200 ${step >= s ? 'bg-primary text-white shadow-card' : 'bg-border text-text-secondary'}`}
                >
                  {step > s ? <Check className="h-4 w-4" /> : s}
                </div>
                {s < 4 && (
                  <div
                    className={`h-0.5 w-8 rounded-full transition-all duration-300 ${step > s ? 'bg-primary' : 'bg-border'}`}
                  />
                )}
              </div>
            ))}
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        <div className="flex flex-1 gap-1">
                          {[1, 2, 3, 4].map((i) => (
                            <div
                              key={i}
                              className={`h-1 flex-1 rounded-full transition-all duration-200 ${i <= pwStrength ? strengthColor : 'bg-border'}`}
                            />
                          ))}
                        </div>
                        <span
                          className={`text-xs font-medium ${pwStrength <= 1 ? 'text-error' : pwStrength === 2 ? 'text-warning' : pwStrength === 3 ? 'text-primary' : 'text-success'}`}
                        >
                          {strengthLabel}
                        </span>
                      </div>
                ...
                {role === 'CANDIDATE' ? (
                  <>
                    ...
                    <p className="mt-2 text-text-secondary">
                      Add the basics recruiters look for first.
                    </p>
                    <div className="mt-6 space-y-4">
                      ...
                      <Textarea
                        ...
                        placeholder="2-3 sentences about your focus, impact, and career goals."
            ...
            {step === 4 && role === 'CANDIDATE' && (
              <div className="animate-fade-in-up space-y-8">
                <div>
                  ...
                  <p className="mt-2 text-text-secondary">
                    This powers your match score and gap analysis.
                  </p>
                </div>
                ...
                <Card className="p-5">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {skillOptions.map((s) => (
                      ...
                      >
                        {skills.includes(s) && <Check className="mr-1 inline h-3.5 w-3.5" />}
                        {s}
                      </button>
                    ))}
                  </div>
                  {skills.length > 0 && (
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                      {skills.map((skill) => (
                        <label
                          key={skill}
                          className="flex items-center justify-between rounded-card border border-border bg-white px-4 py-2 text-sm"
                        >
                    ...
                    {education.map((entry, idx) => (
                      <div key={`${entry.school}-${idx}`} className="grid gap-3 sm:grid-cols-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setEducation((prev) =>
                              prev.map((x, i) => (i === idx ? { ...x, gpa: e.target.value } : x)),
                            )
                          }
                        />
                        <Input
                          ...
                          onChange={(e) =>
                            setEducation((prev) =>
                              prev.map((x, i) =>
                                i === idx ? { ...x, startYear: e.target.value } : x,
                              ),
                            )
                          }
                        />
                        <Input
                          ...
                          onChange={(e) =>
                            setEducation((prev) =>
                              prev.map((x, i) =>
                                i === idx ? { ...x, endYear: e.target.value } : x,
                              ),
                            )
                          }
                        />
                ...
                <Card className="p-5">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      onClick={() => setInternships((prev) => [...prev, { ...emptyInternship }])}
                    >
                      Add internship
                    </Button>
                  </div>
                    ...
                    {projects.map((entry, idx) => (
                      <div key={`${entry.name}-${idx}`} className="grid gap-3 sm:grid-cols-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setProjects((prev) =>
                              prev.map((x, i) => (i === idx ? { ...x, stack: e.target.value } : x)),
                            )
                          }
                        />
                        <Input
