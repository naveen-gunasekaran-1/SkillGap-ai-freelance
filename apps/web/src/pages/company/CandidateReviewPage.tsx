...
import { AlertTriangle, Brain, CheckCircle2, Search, SlidersHorizontal, Sparkles, UserCheck, UserX, X } from 'lucide-react';
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-6xl p-4 lg:p-8">
        ...
        {isVerified && <div className="mt-6 rounded-card border border-border bg-white p-3 shadow-card">
        ...
        {isVerified && <div className="mt-6 grid gap-4">
            ...
            <Card key={application.id} className="p-5">
              <div className="flex flex-col gap-5 md:flex-row md:items-start">
                ...
                <div className="flex flex-wrap items-center gap-4 md:justify-end">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="secondary" disabled={updateStatus.isPending} onClick={() => openRejectModal(application)}>
              ...
              <div className="mt-5 grid gap-4 border-t border-border pt-5 lg:grid-cols-[1fr_280px]">
                <div>
                  ...
                  {topGaps.length > 0 ? (
                    <div className="grid gap-2 sm:grid-cols-2">
                      {topGaps.map((gap) => (
                        <div key={`${application.id}-${gap.skillName}`} className="rounded-card border border-border bg-background/70 p-3">
                          <div className="flex items-center justify-between gap-2">
                            <p className="text-sm font-semibold text-text-primary">{gap.skillName}</p>
                            <Badge variant={gap.severity === 'CRITICAL' ? 'error' : gap.severity === 'MODERATE' ? 'warning' : 'neutral'}>
                              {gap.severity}
                            </Badge>
                          </div>
                          <p className="mt-1 line-clamp-2 text-xs text-text-secondary">{gap.explanation}</p>
                        </div>
                      ))}
                    </div>
                  ) : (
                ...
                <div className="rounded-card border border-ai-purple/20 bg-primary-light/40 p-4">
                  ...
                  <p className="mt-2 text-2xl font-bold text-primary">{gapExplanation?.confidence ?? application.matchScore}%</p>
                  <p className="mt-1 text-xs text-text-secondary">
                    {gapExplanation?.summary ?? 'Match score generated from candidate skills and job requirements.'}
                  </p>
                </div>
              </div>
            </Card>
            ...
            <Card className="max-h-[90vh] w-full max-w-3xl overflow-y-auto p-6 shadow-elevated">
              ...
              <div className="mt-6 rounded-card border border-warning/20 bg-warning/5 p-4">
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-text-primary">Candidate-facing accountability</p>
                    <p className="mt-1 text-sm text-text-secondary">
                      This explanation will be stored in the audit trail and shown to the candidate as improvement guidance.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {rejectionCategories.map((category) => (
                    ...
                    >
                      {selectedCategories.includes(category) && <CheckCircle2 className="mr-1 inline h-3.5 w-3.5" />}
                      {category}
                    </button>
                  ))}
                </div>
              ...
              <div className="mt-6">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {[
                    ...(selectedApplication.aiExplanations?.find((e) => e.type === 'GAP_REPORT')?.missingSkills ?? []),
                    ...(selectedApplication.aiExplanations?.find((e) => e.type === 'GAP_REPORT')?.weakEvidence ?? []),
                    ...
                    .map((skill) => (
                      ...
                      >
                        {selectedSkills.includes(skill) && <CheckCircle2 className="mr-1 inline h-3.5 w-3.5" />}
                        {skill}
                      </button>
                    ))}
                </div>
              ...
              <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                <Button type="button" variant="ai-gradient" disabled={updateStatus.isPending} onClick={submitRejection}>
